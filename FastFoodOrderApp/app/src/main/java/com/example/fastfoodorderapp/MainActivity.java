...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        navView.setNavigationItemSelectedListener(item -> {
            ...
            if (id == R.id.nav_inicio) {
                ...
                // 1. LIMPIAR SESIÓN (si usas SharedPreferences)
                getSharedPreferences("sesion", MODE_PRIVATE)
                        .edit()
                        .clear()
                        .apply();

                // 2. IR A LOGIN Y LIMPIAR TODA LA PILA DE ACTIVITIES
                Intent intent = new Intent(MainActivity.this, LoginActivity.class);
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);

                startActivity(intent);

                // 3. IMPORTANTE: cerrar MainActivity completamente
                finish();
    ...
    private void mostrarHistorialPedidos() {
        ...
        String historial = "📋 Historial de Pedidos\n\n";

        for (Pedido pedido : dbHelper.obtenerPedidos()) {

            historial += "🕒 " + pedido.getFecha() + "\n";

            if (pedido.getHamburguesa() > 0)
                historial += "🍔 Hamburguesa x" + pedido.getHamburguesa() + "\n";

            if (pedido.getPapas() > 0)
                historial += "🍟 Papas x" + pedido.getPapas() + "\n";

            if (pedido.getRefresco() > 0)
                historial += "🥤 Refresco x" + pedido.getRefresco() + "\n";

            if (pedido.getHelado() > 0)
                historial += "🍦 Helado x" + pedido.getHelado() + "\n";

            historial += "💲 Total: $" + pedido.getTotal() + "\n\n";
        }

        if (dbHelper.obtenerPedidos().isEmpty()) {
            historial += "No hay pedidos guardados";
        }
